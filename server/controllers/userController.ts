...
export const createUserProject = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const promptEnhanceResult = await model.generateContent(`${promptEnhanceSystemPrompt}\n\nUser request: ${initial_prompt}`);
        const enhancedPrompt = await promptEnhanceResult.response.text();
        ...
        const codeGenerationResult = await model.generateContent(`${codeGenerationSystemPrompt}\n\n${enhancedPrompt || ''}`);
...
export const purchaseCredits = async (req: Request, res: Response) => {
    ...
    try {
        res.json({ message: 'Purchase credits endpoint' });
    } catch (error: any) {
        console.log(error.code || error.message);
        res.status(500).json({ message: error.message });
    }
